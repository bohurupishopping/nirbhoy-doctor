...
class PrescriptionPreviewSheet extends ConsumerStatefulWidget {
  final String appointmentId;

  const PrescriptionPreviewSheet({super.key, required this.appointmentId});
  ...
  static void show(BuildContext context, String appointmentId) {
    showModalBottomSheet(
      ...
      builder: (context) =>
          PrescriptionPreviewSheet(appointmentId: appointmentId),
    );
  }

  @override
  ConsumerState<PrescriptionPreviewSheet> createState() =>
      _PrescriptionPreviewSheetState();
}

class _PrescriptionPreviewSheetState
    extends ConsumerState<PrescriptionPreviewSheet> {
  // We fetch header info (Clinic/Doctor) asynchronously
  late Future<Map<String, dynamic>> _headerFuture;

  @override
  void initState() {
    super.initState();
    final repo = ref.read(consultationRepositoryProvider);
    _headerFuture = repo.getPrescriptionHeaderData(widget.appointmentId);
  }
            ...
            child: FutureBuilder<Map<String, dynamic>>(
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) {
                  return Center(
                    child: Text('Error loading preview: ${snapshot.error}'),
                  );
                }
                ...
                // Merge Snapshot Data with Consultation State
                final headerData = snapshot.data!;
                final printData = _buildPrintData(headerData, state);

                return SingleChildScrollView(
                  padding: const EdgeInsets.all(24),
                  child: _buildA4Prescription(context, printData),
                );
              },
            ),
          ),
  ...
  PrescriptionPrintData _buildPrintData(
    ...
    return PrescriptionPrintData(
      ...
      clinical: PrescriptionClinical(
        ...
        notes: null, // or add if available in state
      ),
      rxItems: state.medicines
          .map(
      ...
      advice: PrescriptionAdvice(
        nextVisitDate: state.nextVisitDate?.toString().split(' ')[0],
        nextVisitText: state.adviceNotes,
      ),
    );
  }

  // --- UI Builder (Copied & Adapted from PrescriptionPrintView) ---
