...
\restrict CZh2b5R9RD6um9kIo2ijtm3h41IcgSiwkzbzN0xrx92SXVfmIyHKRb96yAtpcfi
...
--
-- Name: _internal_get_notification_meta(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public._internal_get_notification_meta(_appt_id uuid) RETURNS jsonb
...
BEGIN
    SELECT jsonb_build_object(
        ...
        'patient_uhid', COALESCE(p.uhid, ''), -- ✅ Added for History Link
        'doctor_name', dr_p.full_name,
        'clinic_name', c.name,
        'clinic_phone', c.phone_primary,
        'clinic_city', c.city,
        'appt_date_formatted', to_char(a.start_time, 'DD Mon YYYY'),
        'appt_time_formatted', to_char(a.start_time, 'HH12:MI AM')
    )
    INTO _result
    FROM appointments a
...
--
-- Name: analyze_doctor_patterns(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.analyze_doctor_patterns(_doctor_id uuid) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
    _clinic_id uuid;
    _diagnosis_stats jsonb;
    _rx_habits jsonb;
BEGIN
    SELECT clinic_id INTO _clinic_id FROM profiles WHERE id = _doctor_id;

    -- Top 20 Diagnoses
    SELECT jsonb_agg(t) INTO _diagnosis_stats
    FROM (
        SELECT value::text as diagnosis, count(*) as freq
        FROM consultations c
        JOIN appointments a ON c.appointment_id = a.id,
        jsonb_array_elements_text(c.diagnosis_codes) as value
    ...
    -- Prescribing Habits (Conditions > 5 occurences)
    SELECT jsonb_object_agg(diagnosis, meds) INTO _rx_habits
    FROM (
        SELECT 
            d.value::text as diagnosis,
            jsonb_agg(DISTINCT pi.medicine_name_manual) as meds
        FROM consultations c
        JOIN appointments a ON c.appointment_id = a.id
        JOIN prescription_items pi ON pi.consultation_id = c.id,
        jsonb_array_elements_text(c.diagnosis_codes) as d
        WHERE a.doctor_id = _doctor_id
    ...
    INSERT INTO doctor_clinical_patterns (doctor_id, clinic_id, top_diagnoses, prescribing_habits, last_analyzed_at)
    VALUES (_doctor_id, _clinic_id, COALESCE(_diagnosis_stats, '[]'::jsonb), COALESCE(_rx_habits, '{}'::jsonb), NOW())
    ON CONFLICT (doctor_id) 
    DO UPDATE SET 
        top_diagnoses = EXCLUDED.top_diagnoses,
        prescribing_habits = EXCLUDED.prescribing_habits,
...
CREATE FUNCTION public.book_appointment_atomic(_patient_id uuid, _doctor_id uuid, _clinic_id uuid DEFAULT NULL::uuid, _slot_time timestamp with time zone DEFAULT NULL::timestamp with time zone, _consult_fee numeric DEFAULT 0) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
    ...
    _final_clinic_id := COALESCE(_clinic_id, _requester_clinic_id);

    IF _final_clinic_id IS NULL THEN
        RAISE EXCEPTION 'Clinic ID is missing.';
    END IF;

    IF _requester_clinic_id IS NOT NULL 
       AND _requester_clinic_id IS DISTINCT FROM _final_clinic_id 
    ...
    VALUES (
        ...
        _doctor_id, 
        _slot_time, 
        _slot_time + (COALESCE((SELECT avg_consult_time_min FROM doctors WHERE profile_id=_doctor_id), 15)) * interval '1 minute',
...
--
-- Name: charge_wallet_for_message(uuid, numeric, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.charge_wallet_for_message(_clinic_id uuid, _cost numeric, _message_type text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
BEGIN
    ...
    IF _current_balance < _cost THEN
        RETURN jsonb_build_object('success', false, 'error', 'insufficient_funds', 'balance', _current_balance); 
    END IF;
    ...
    INSERT INTO public.clinic_wallet_transactions (
        wallet_id, amount, type, reference_id, description, balance_after
    ) VALUES (
        _wallet_id, -_cost, 'usage_deduction', NULL, 'WhatsApp: ' || _message_type, _new_balance
    ) RETURNING id INTO _txn_id;

    RETURN jsonb_build_object('success', true, 'new_balance', _new_balance, 'transaction_id', _txn_id);
END;
$$;
...
--
-- Name: create_clinic_invitation(text, public.user_role); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.create_clinic_invitation(_email text, _role public.user_role) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
BEGIN
    ...
    _clean_email := TRIM(LOWER(_email));

    -- 3. Validation: Is this user ALREADY in this clinic?
    IF EXISTS (SELECT 1 FROM profiles WHERE email = _clean_email AND clinic_id = _clinic_id) THEN
        RETURN jsonb_build_object('success', false, 'error', 'User is already a member of this clinic.');
    END IF;

    -- 4. Create/Update Invitation
    -- If an expired invite exists, we refresh it.
    INSERT INTO public.clinic_invitations (clinic_id, email, role, invited_by, status, expires_at)
    VALUES (_clinic_id, _clean_email, _role, auth.uid(), 'pending', (now() + interval '7 days'))
    ...
    RETURN jsonb_build_object('success', true, 'message', 'Invitation created successfully.');
END;
$$;
...
BEGIN
    -- 1. Security Check
    IF public.auth_user_role() NOT IN ('admin', 'doctor', 'super_admin') THEN
    ...
    INSERT INTO public.medicine_master (
        name, manufacturer_name, salt_composition, short_composition1, short_composition2, 
        pack_size_label, price, type, medicine_desc, side_effects, is_discontinued
    ) VALUES (
        TRIM(_name), TRIM(_manufacturer_name), TRIM(_salt_composition), TRIM(_short_composition1), TRIM(_short_composition2), 
        TRIM(_pack_size_label), COALESCE(_price, 0), TRIM(_type), TRIM(_medicine_desc), TRIM(_side_effects), false
    )
...
CREATE FUNCTION public.doctor_call_patient(_appointment_id uuid, _doctor_id uuid DEFAULT NULL::uuid) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
    ...
    -- 1. Get Appointment Owner
    SELECT doctor_id INTO _appt_doctor_id FROM appointments WHERE id = _appointment_id;

    -- 2. Authorization Check
    -- Allow if: User is the Doctor OR User is Admin
    IF _caller_id != _appt_doctor_id AND public.auth_user_role() != 'admin' THEN
        RAISE EXCEPTION 'Unauthorized: You cannot call patients for another doctor.';
    END IF;
    ...
    -- (Trigger `trg_sync_appt_status` handles the Appointment status update automatically)
END;
$$;
...
--
-- Name: enhanced_audit_log(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.enhanced_audit_log() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
BEGIN
    ...
    -- 2. CAPTURE NETWORK CONTEXT
    BEGIN
        _headers := current_setting('request.headers', true)::jsonb;
        _raw_ip := COALESCE(_headers ->> 'x-forwarded-for', '127.0.0.1');
        _ip_address := TRIM(split_part(_raw_ip, ',', 1));
        _user_agent := _headers ->> 'user-agent';
    EXCEPTION WHEN OTHERS THEN
        _ip_address := 'internal';
        _user_agent := 'system_process';
    END;

    -- 3. SAFE DATA EXTRACTION (Fixing the "no field id" error)
    -- We convert records to JSONB first. accessing non-existent keys in JSONB returns NULL 
    -- instead of throwing an error.
    
    IF (TG_OP = 'DELETE') THEN
        _old_data := to_jsonb(OLD);
        -- Try 'id', if null, try 'profile_id' (for doctors table), if null, ignore.
        _resource_id := COALESCE(
            (_old_data ->> 'id')::uuid, 
            (_old_data ->> 'profile_id')::uuid
        );
        
        IF (_old_data ? 'clinic_id') THEN 
            _clinic_id := (_old_data ->> 'clinic_id')::uuid; 
        END IF;
    ELSE
        _new_data := to_jsonb(NEW);
        _resource_id := COALESCE(
            (_new_data ->> 'id')::uuid, 
            (_new_data ->> 'profile_id')::uuid
        );

        IF (_new_data ? 'clinic_id') THEN 
            _clinic_id := (_new_data ->> 'clinic_id')::uuid; 
        END IF;
    END IF;

    -- Fallback for Clinic ID (e.g. Doctors table doesn't have clinic_id, but the user does)
    IF _clinic_id IS NULL AND _user_id IS NOT NULL THEN
        SELECT clinic_id INTO _clinic_id FROM public.profiles WHERE id = _user_id;
    END IF;

    -- 4. SMART DIFFING
    IF TG_OP = 'UPDATE' THEN
        SELECT jsonb_object_agg(key, value) INTO _changes
        FROM jsonb_each(_new_data)
        WHERE to_jsonb(OLD) -> key IS DISTINCT FROM value
        AND key NOT IN ('updated_at', 'search_vector', 'created_at');
        
        IF _changes IS NULL THEN RETURN NEW; END IF;
        ...
        _changes := jsonb_build_object(
            ...
            'previous_values', (
                ...
                WHERE to_jsonb(OLD) -> key IS DISTINCT FROM _new_data -> key
            )
        );
    ELSIF TG_OP = 'INSERT' THEN
        _changes := jsonb_build_object('new_data', _new_data);
    ELSE 
        _changes := jsonb_build_object('deleted_data', _old_data);
        _severity := 'warning'; 
    END IF;

    -- 5. AUTO-CATEGORIZATION
    CASE TG_TABLE_NAME
        WHEN 'patients', 'consultations', 'prescription_items', 'lab_orders', 'doctors', 'doctor_schedules' THEN 
            _category := 'clinical';
        WHEN 'invoices', 'invoice_payments', 'invoice_items', 'clinic_wallets' THEN 
            _category := 'financial';
        WHEN 'profiles', 'auth.users', 'clinic_api_keys' THEN 
            _category := 'security';
            IF TG_OP = 'DELETE' THEN _severity := 'critical'; END IF;
        WHEN 'clinics', 'clinic_subscriptions' THEN
            _category := 'administrative';
        ELSE 
            _category := 'general';
    END CASE;
...
DECLARE
    ...
    _transactions jsonb;
BEGIN
    _final_clinic_id := COALESCE(_clinic_id, public.auth_my_clinic_id());
    ...
    WITH filtered_invoices AS (
        SELECT 
            ...
            -- LOGIC: Refunded items count as 0 collected
            CASE WHEN i.payment_status = 'refunded' THEN 0 ELSE i.amount_paid END as effective_collected,
            i.amount_paid as raw_paid,
            i.payment_status, i.payment_mode,
            (i.grand_total - i.amount_paid) as balance_due,
            p.full_name as patient_name, p.uhid, p.phone,
            dr_p.full_name as doctor_name, dr.specialty,
        ...
        WHERE 
            ...
            AND (
                ...
                p.full_name ILIKE '%' || _search_text || '%'
            )
    ),
    stats_agg AS (
        SELECT 
            ...
            COALESCE(SUM(effective_collected), 0) as total_net_collected,
    ...
    chart_doctor AS (
        SELECT jsonb_agg(t) as data FROM (
            ...
            FROM filtered_invoices GROUP BY doctor_name ORDER BY total DESC
        ) t
    ),
    chart_mode AS (
        SELECT jsonb_agg(t) as data FROM (
            SELECT COALESCE(payment_mode, 'Unpaid') as mode, SUM(effective_collected) as total
            FROM filtered_invoices WHERE effective_collected > 0 GROUP BY payment_mode ORDER BY total DESC
        ) t
    ),
    chart_trend AS (
        SELECT jsonb_agg(t) as data FROM (
            SELECT created_at::DATE as date, SUM(grand_total) as billed, SUM(effective_collected) as collected
            FROM filtered_invoices GROUP BY created_at::DATE ORDER BY created_at::DATE ASC
        ) t
    )
    SELECT 
        jsonb_build_object(
        ...
        jsonb_build_object(
            ...
            'daily_trend', COALESCE((SELECT data FROM chart_trend), '[]'::jsonb)
        ),
        COALESCE(jsonb_agg(
            jsonb_build_object(
                'id', f.id,
                ...
                'financials', jsonb_build_object('total', f.grand_total, 'paid', f.raw_paid, 'due', f.balance_due),
    ...
    FROM (
        SELECT * FROM filtered_invoices ORDER BY created_at DESC LIMIT _page_size OFFSET _offset
    ) f;

    RETURN jsonb_build_object(
        'meta', jsonb_build_object('page', _page, 'size', _page_size, 'total_records', COALESCE(_total_count, 0)),
        'summary', _stats,
        'analytics', _charts,
        'data', _transactions
    );
...
BEGIN
    -- A. Zero-Latency Context
    _user_clinic_id := public.auth_my_clinic_id();

    -- B. Single Query Fetch (Everything in one go)
    SELECT 
        jsonb_build_object(
            'appointment', jsonb_build_object(
                ...
                'clinic_id', a.clinic_id, -- Needed for security check
                
                -- Expanded Patient Metadata (Saves UI fetching profile separately)
                'patient_id', p.id,
                'patient_name', p.full_name,
                'patient_phone', p.phone,
                'patient_uhid', p.uhid,
                ...
                -- Workflow Context (Crucial for UI Logic)
                'consultation_id', c.id -- If NOT NULL, UI shows "Resume", else "Start"
            ),
            'invoice', CASE WHEN i.id IS NULL THEN NULL ELSE 
                jsonb_build_object(
                    'id', i.id,
                    'invoice_number', i.invoice_number,
            ...
            'queue_token', CASE WHEN q.id IS NULL THEN NULL ELSE 
                jsonb_build_object(
                    'token_number', q.token_number, 
                    'status', q.status,
    ...
    LEFT JOIN doctors d ON d.profile_id = dr.id
    LEFT JOIN invoices i ON i.appointment_id = a.id
    LEFT JOIN queue_tokens q ON q.appointment_id = a.id
    LEFT JOIN consultations c ON c.appointment_id = a.id
    WHERE a.id = _appointment_id;

    -- C. Logic & Security Checks
    IF _data IS NULL THEN
        RAISE EXCEPTION 'Appointment not found';
    END IF;

    -- Secure Clinic Check (Memory Compare)
    IF (_data->'appointment'->>'clinic_id')::uuid IS DISTINCT FROM _user_clinic_id
       AND public.auth_user_role() != 'super_admin' THEN
        RAISE EXCEPTION 'Unauthorized: Appointment belongs to another clinic.';
    END IF;
...
CREATE FUNCTION public.get_appointments_grid(_clinic_id uuid DEFAULT NULL::uuid, _date date DEFAULT CURRENT_DATE) RETURNS jsonb
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
    _resolved_id uuid;
BEGIN
    _resolved_id := COALESCE(_clinic_id, public.auth_my_clinic_id());

    IF public.auth_user_role() NOT IN ('admin', 'staff', 'super_admin') THEN
        RAISE EXCEPTION 'Unauthorized: Doctors must use personal grid.';
    END IF;

    IF _resolved_id IS NULL THEN RAISE EXCEPTION 'Context missing'; END IF;

    RETURN (
        SELECT COALESCE(jsonb_agg(t), '[]'::jsonb) FROM (
            SELECT 
                a.id, a.start_time, a.end_time, a.status, a.type,
                p.full_name as patient_name, p.uhid,
                dr_p.full_name as doctor_name, a.doctor_id
            FROM appointments a
            JOIN patients p ON a.patient_id = p.id
            JOIN profiles dr_p ON a.doctor_id = dr_p.id
            WHERE a.clinic_id = _resolved_id 
...
CREATE FUNCTION public.get_booking_init_data(_clinic_id uuid DEFAULT NULL::uuid) RETURNS jsonb
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
    _requester_clinic_id := public.auth_my_clinic_id();
    _target_clinic_id := COALESCE(_clinic_id, _requester_clinic_id);

    -- Security Guard
    IF _requester_clinic_id IS DISTINCT FROM _target_clinic_id 
       AND public.auth_user_role() != 'super_admin' THEN
        RETURN jsonb_build_object('doctors', '[]'::jsonb);
    END IF;
    ...
    WHERE p.clinic_id = _target_clinic_id
    AND p.role = 'doctor'
...
CREATE FUNCTION public.get_doctor_active_queue(_doctor_id uuid DEFAULT NULL::uuid) RETURNS TABLE(token_number integer, patient_name text, age integer, gender text, uhid text, appointment_id uuid, queue_status text, payment_status public.payment_status, amount_due numeric, access_flags jsonb, checked_in_at timestamp with time zone, wait_time_mins integer)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
    _target_id uuid;
BEGIN
    _target_id := COALESCE(_doctor_id, auth.uid());

    -- Security Guard
    IF _target_id != auth.uid() AND public.auth_user_role() NOT IN ('admin', 'super_admin') THEN
        RAISE EXCEPTION 'Unauthorized.';
    END IF;
    ...
    SELECT 
        ...
        p.access_flags,
        a.checked_in_at,
        -- Calculate wait time safely (in minutes)
...
CREATE FUNCTION public.get_doctor_analytics(_doctor_id uuid DEFAULT NULL::uuid) RETURNS jsonb
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
    ...
    SELECT COUNT(*) INTO _patients_today
    FROM appointments
    WHERE doctor_id = _target_id
    AND status = 'completed'
    AND start_time::DATE = CURRENT_DATE;

    -- 2. Calculate Average Consult Time (Last 30 days)
...
BEGIN
    ...
    -- Ensure only doctors call this (or add logic for admins if needed)
    IF public.auth_user_role() NOT IN ('doctor', 'admin', 'super_admin') THEN
        RAISE EXCEPTION 'Unauthorized.';
    END IF;
    ...
    total_count_cte AS ( SELECT COUNT(*) as total FROM filtered_data )
    SELECT 
        jsonb_build_object(
    ...
    INTO _result
    FROM (
        SELECT * FROM filtered_data ORDER BY start_time DESC LIMIT _page_size OFFSET _offset
    ) f;
...
CREATE FUNCTION public.get_doctor_dashboard_metrics(_doctor_id uuid DEFAULT NULL::uuid) RETURNS TABLE(waiting_count bigint, completed_today bigint, avg_consult_time_mins numeric, avg_wait_time_mins numeric)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
    _target_id uuid;
BEGIN
    -- Resolve Target: Default to self, allow Admin to peek
    _target_id := COALESCE(_doctor_id, auth.uid());

    -- Security Guard
    IF _target_id != auth.uid() AND public.auth_user_role() NOT IN ('admin', 'super_admin') THEN
        RAISE EXCEPTION 'Unauthorized: You can only view your own metrics.';
    END IF;
    ...
    SELECT 
        ...
        -- 3. Avg Consult Duration (Last 30 days)
        (SELECT COALESCE(ROUND(AVG(EXTRACT(EPOCH FROM (consult_ended_at - consult_started_at)) / 60)::numeric, 1), 0)
         FROM appointments
         WHERE doctor_id = _target_id
           AND status = 'completed'
...
CREATE FUNCTION public.get_doctors_v2(_clinic_id uuid DEFAULT NULL::uuid, _search text DEFAULT NULL::text) RETURNS TABLE(doctor_id uuid, full_name text, specialty text, consult_fee numeric, avg_consult_time_min integer, is_available boolean)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
    ...
    WHERE p.clinic_id = _resolved_id
      AND p.role = 'doctor'
      AND (
...
CREATE FUNCTION public.get_live_queue_monitor(_clinic_id uuid DEFAULT NULL::uuid) RETURNS jsonb
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
    ...
    IF _resolved_id IS NULL THEN RAISE EXCEPTION 'Clinic context missing.'; END IF;

    RETURN jsonb_build_object(
      'scheduled', (
          SELECT COALESCE(jsonb_agg(t), '[]'::jsonb) FROM (
              SELECT q.appointment_id, p.full_name, p.phone, p.uhid,
                to_char(a.start_time, 'HH12:MI AM') as slot_time, d.full_name as doctor_name
              FROM queue_tokens q
              JOIN patients p ON q.patient_id = p.id
              JOIN appointments a ON q.appointment_id = a.id
              JOIN profiles d ON q.doctor_id = d.id
              WHERE q.clinic_id = _resolved_id 
                AND q.date_of_service = CURRENT_DATE 
      ...
      'waiting', (
          SELECT COALESCE(jsonb_agg(t), '[]'::jsonb) FROM (
              SELECT q.id as token_id, q.appointment_id, q.token_number, 
                p.full_name, p.uhid, p.access_flags, d.full_name as doctor_name,
      ...
      'active', (
          SELECT COALESCE(jsonb_agg(t), '[]'::jsonb) FROM (
               SELECT q.appointment_id, q.token_number, p.full_name, 
                d.full_name as doctor_name, q.room_number,
...
BEGIN
    ...
    SELECT jsonb_build_object(
        ...
        -- C. DOCTOR CONTEXT (For AI Persona)
        'doctor_context', (
            SELECT jsonb_build_object(
                'frequent_rx', prescribing_habits,
                'common_diagnoses', top_diagnoses,
        ...
        -- D. DOCUMENT HISTORY (HYBRID: Legacy + AI)
        -- ✅ FIX: Restored original key 'documents' and original fields
        'documents', (
            SELECT COALESCE(jsonb_agg(
                jsonb_build_object(
                    -- 1. Legacy Fields (For UI List)
                    'id', d.id,
                    'name', d.file_name,
                    'url', d.file_url,
                    'category', d.category,
                    ...
                    -- 2. New AI Fields (For Intelligence)
                    'ai_summary', dc.ai_summary,
                    'key_findings', dc.clinical_data, 
                    'tags', dc.semantic_tags
                ) ORDER BY d.uploaded_at DESC
            ), '[]'::jsonb)
            FROM patient_documents d
            LEFT JOIN profiles u_p ON d.uploaded_by = u_p.id
            LEFT JOIN patient_document_content dc ON d.id = dc.document_id
        ...
        'visit_history', (
            SELECT COALESCE(jsonb_agg(
                jsonb_build_object(
                    ...
                    'prescriptions', (
                        SELECT COALESCE(jsonb_agg(
                            medicine_name_manual || ' (' || frequency || ')'
                        ), '[]'::jsonb)
                        FROM prescription_items pi WHERE pi.consultation_id = c.id
                    ),
                    'labs', (
                        SELECT COALESCE(jsonb_agg(test_name), '[]'::jsonb)
                        FROM lab_orders lo WHERE lo.consultation_id = c.id
                    )
                ) ORDER BY a.start_time DESC
            ), '[]'::jsonb)
            FROM appointments a
...
BEGIN
    ...
    -- B. Single-Query Execution Strategy using CTEs
    WITH 
    -- 1. Fetch Patient & Verify Security in one go
    target_patient AS (
        SELECT * 
        FROM patients 
        WHERE id = _patient_id
    ),
    -- 2. Aggregated Financial Stats (Super Fast Server-Side Math)
    patient_stats AS (
        SELECT 
            COUNT(*) FILTER (WHERE status = 'completed') as visited_count,
            COUNT(*) FILTER (WHERE status = 'no_show') as no_show_count,
            COALESCE(SUM(i.grand_total), 0) as total_billed,
            COALESCE(SUM(i.grand_total - i.amount_paid), 0) as total_due
        FROM appointments a
        LEFT JOIN invoices i ON a.id = i.appointment_id
        WHERE a.patient_id = _patient_id 
        AND a.status != 'cancelled'
    ),
    -- 3. Fetch Appointments (The History)
    appt_list AS (
        SELECT jsonb_agg(
            jsonb_build_object(
                'appointment_id', a.id,
    ...
    -- C. Construct Final JSON Response
    SELECT jsonb_build_object(
        'profile', (
            SELECT to_jsonb(tp) || jsonb_build_object(
                -- Inject calculated stats directly into the profile object for easy UI access
                'stats', (SELECT to_jsonb(ps) FROM patient_stats ps)
            ) 
            FROM target_patient tp
        ),
        'appointments', COALESCE((SELECT data FROM appt_list), '[]'::jsonb),
        'documents', COALESCE((SELECT data FROM doc_list), '[]'::jsonb)
    ) INTO _result
    FROM target_patient tp;

    -- D. Security Check: If no patient found or clinic mismatch
    IF _result IS NULL OR (_result->>'profile') IS NULL THEN
        -- Check if it was a permission issue or just not found
        IF EXISTS (SELECT 1 FROM patients WHERE id = _patient_id) THEN
    ...
    -- Additional Clinic ID Verification (Double Check)
    IF ((_result->'profile'->>'clinic_id')::uuid IS DISTINCT FROM _user_clinic_id) 
       AND public.auth_user_role() != 'super_admin' THEN
...
CREATE FUNCTION public.get_patient_followups(_clinic_id uuid DEFAULT NULL::uuid, _start_date date DEFAULT CURRENT_DATE, _end_date date DEFAULT CURRENT_DATE, _page integer DEFAULT 1, _page_size integer DEFAULT 20) RETURNS TABLE(patient_id uuid, patient_name text, phone text, uhid text, address text, last_visit_date timestamp with time zone, next_visit_due_date date, next_visit_notes text, doctor_name text, is_already_booked boolean, total_count bigint)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
BEGIN
    ...
    WITH filtered_data AS (
        SELECT 
            p.id AS p_id, p.full_name, p.phone, p.uhid,
            TRIM(BOTH ', ' FROM CONCAT_WS(', ', NULLIF(TRIM(p.address_street), ''), NULLIF(TRIM(p.district), ''), NULLIF(TRIM(p.state), ''))) AS formatted_address,
            a.start_time AS last_visit_ts, 
            c.next_visit_date, c.next_visit_text, dr_p.full_name AS doc_name,
            EXISTS (
                SELECT 1 FROM appointments future_a
                WHERE future_a.patient_id = p.id AND future_a.clinic_id = _resolved_id
                AND future_a.start_time > a.start_time AND future_a.status NOT IN ('cancelled', 'no_show')
            ) as booked_status
        FROM consultations c
        JOIN appointments a ON c.appointment_id = a.id
        JOIN patients p ON a.patient_id = p.id
    ...
    total_calc AS ( SELECT COUNT(*) as count FROM filtered_data )
    SELECT f.p_id, f.full_name, f.phone, f.uhid, f.formatted_address,
           f.last_visit_ts, f.next_visit_date, f.next_visit_text, f.doc_name, f.booked_status, t.count
    FROM filtered_data f, total_calc t
    ORDER BY f.next_visit_date ASC, f.last_visit_ts DESC
    LIMIT _page_size OFFSET _offset;
END;
$$;
...
BEGIN
    -- Scope Check
    SELECT clinic_id INTO _clinic_id FROM patients WHERE id = _patient_id;
    
    IF _clinic_id IS DISTINCT FROM public.auth_my_clinic_id() AND public.auth_user_role() != 'super_admin' THEN
        RAISE EXCEPTION 'Unauthorized.';
    END IF;
...
CREATE FUNCTION public.get_patients_v2(_clinic_id uuid DEFAULT NULL::uuid, _search text DEFAULT NULL::text, _gender public.gender_type DEFAULT NULL::public.gender_type, _page integer DEFAULT 1, _page_size integer DEFAULT 12) RETURNS TABLE(id uuid, full_name text, phone text, uhid text, gender public.gender_type, age integer, created_at timestamp with time zone, total_count bigint)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
    ...
    _offset integer;
BEGIN
    -- A. Zero-Latency Context Resolution
    _resolved_id := COALESCE(_clinic_id, public.auth_my_clinic_id());
    ...
    -- C. Security Guard
    IF public.auth_user_role() NOT IN ('admin', 'staff', 'doctor', 'super_admin') THEN
        RAISE EXCEPTION 'Unauthorized Access';
    END IF;

    IF _resolved_id IS NULL THEN 
        RAISE EXCEPTION 'Clinic context missing.'; 
    END IF;

    -- D. Perform Query with Window Function for Total Count
    RETURN QUERY
    WITH filtered_patients AS (
        ...
        WHERE p.clinic_id = _resolved_id
          ...
          -- Search Filter (Matches Name, Phone, or UHID)
          AND (
            _search IS NULL OR _search = '' OR
            p.full_name ILIKE '%' || _search || '%' OR
    ...
    total_calc AS (
        -- Calculate total count of the filtered set ONCE
        SELECT COUNT(*) as exact_count FROM filtered_patients
    )
    SELECT 
        f.id, 
        f.full_name, 
    ...
    RETURN (
        SELECT jsonb_build_object(
            ...
            'meta', jsonb_build_object(
                ...
                'appointment_id', a.id,
                'visit_time', to_char(a.start_time, 'HH12:MI AM')
            ),
            ...
            'doctor', jsonb_build_object(
                ...
                'hospital_attachment', dr.current_hospital_attachment,
                'phone', dr.phone_number -- ✅ Added Phone Number
            ),
            ...
            'rx_items', (
                SELECT COALESCE(jsonb_agg(
                    jsonb_build_object(
                        ...
                        'generic_name', CASE 
                            WHEN pi.cached_composition IS NOT NULL AND pi.cached_composition != '' THEN pi.cached_composition
                            ELSE TRIM(BOTH ' ' FROM CONCAT_WS(' + ', NULLIF(TRIM(m.short_composition1), ''), NULLIF(TRIM(m.short_composition2), '')))
                        END,
                        'type', COALESCE(m.type, 'Tab'),
                        'dosage', pi.frequency,
                        'duration', pi.duration,
                        'instruction', pi.instructions_timing,
            ...
            'lab_orders', (
                SELECT COALESCE(jsonb_agg(
                    jsonb_build_object(
                        ...
                        'status', CASE WHEN lo.is_completed THEN 'Completed' ELSE 'Pending' END
                    )
                ), '[]'::jsonb)
                FROM lab_orders lo
                WHERE c.id IS NOT NULL AND lo.consultation_id = c.id
            ...
            'advice', jsonb_build_object(
                ...
                'next_visit_text', c.next_visit_text,
                'next_visit_days_left', CASE 
                    WHEN c.next_visit_date IS NOT NULL THEN (c.next_visit_date - CURRENT_DATE) 
                    ELSE NULL 
                END
            )
        )
...
CREATE FUNCTION public.get_staff_dashboard_stats(_clinic_id uuid DEFAULT NULL::uuid) RETURNS jsonb
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
    ...
    FROM appointments
    WHERE clinic_id = _resolved_id 
    AND start_time::DATE = CURRENT_DATE
    AND status != 'cancelled';

    -- 2. Avg Wait Time (Only for checked-in patients)
...
DECLARE
    ...
    _final_role public.user_role;
    _final_clinic_id uuid;
BEGIN
    -- 1. Normalize Email
    -- Supabase Auth usually stores lowercase, but we ensure consistency
    
    -- 2. Check for a valid, pending invitation
    SELECT * INTO _invite_record
    FROM public.clinic_invitations
    WHERE email = LOWER(NEW.email) 
    AND status = 'pending'
    AND expires_at > now()
    ORDER BY created_at DESC -- In case of multiples (rare), take latest
    LIMIT 1;

    -- 3. Determine Role and Clinic
    IF _invite_record.id IS NOT NULL THEN
        -- Case A: User was invited. Use those details.
        _final_role := _invite_record.role;
        _final_clinic_id := _invite_record.clinic_id;
    ...
    ELSE
        -- Case B: Random signup (No invite).
        -- Set to 'user' and NULL clinic (or block them if your app is private)
        _final_role := 'user';
        _final_clinic_id := NULL;
    END IF;
    ...
    -- 5. Sync Metadata to Auth (Optional but recommended for frontend access)
    UPDATE auth.users
    SET raw_app_meta_data = 
        COALESCE(raw_app_meta_data, '{}'::jsonb) || 
        jsonb_build_object(
            'role', _final_role, 
            'clinic_id', _final_clinic_id
        )
    WHERE id = NEW.id;
...
--
-- Name: handle_wallet_deduction_on_log(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.handle_wallet_deduction_on_log() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
    _wallet_id uuid;
    ...
    _new_balance numeric;
BEGIN
    -- 1. Validation: Only run if there is a cost involved
    IF NEW.cost IS NULL OR NEW.cost <= 0 THEN
        RETURN NEW;
    END IF;

    -- 2. Find the Clinic's Wallet
    SELECT id, balance INTO _wallet_id, _current_balance
    FROM public.clinic_wallets
    WHERE clinic_id = NEW.clinic_id;

    IF _wallet_id IS NULL THEN
        RAISE EXCEPTION 'Wallet not found for clinic %', NEW.clinic_id;
    END IF;

    -- 3. Deduct Balance (Allow overdraft? No, we enforced check >= 0)
    -- If balance is insufficient, this UPDATE will fail due to the constraint,
    -- causing the entire Log Insert to roll back. 
    UPDATE public.clinic_wallets
    SET 
        balance = balance - NEW.cost, 
        updated_at = now()
    WHERE id = _wallet_id
    RETURNING balance INTO _new_balance;

    -- 4. Create the Ledger Entry (The Receipt)
    INSERT INTO public.clinic_wallet_transactions (
        wallet_id, 
        amount, 
        type, 
        reference_id, -- We link this transaction back to the Log ID
        description, 
        balance_after, 
        created_at
    ) VALUES (
        _wallet_id, 
        -(NEW.cost), -- Negative amount for deduction
        'usage_deduction', 
        NEW.id::text, -- Link to the Communication Log ID
        'Message Charge: ' || NEW.type, -- e.g. "Message Charge: APPT_BOOKED"
        _new_balance, 
        now()
    );
...
DECLARE
    ...
    _meta JSONB;
    _payload JSONB;
    _history_suffix TEXT;
    _pdf_path TEXT;  -- ✅ NEW: Full path for Var4
BEGIN
    IF NEW.status = 'completed'::public.appointment_status 
       AND (OLD.status IS DISTINCT FROM 'completed'::public.appointment_status) THEN
        ...
        -- A. Gatekeepers
        SELECT (status = 'active' AND ends_at > NOW()), limit_sms_enabled INTO _sub_active, _sms_enabled
        FROM clinic_subscriptions WHERE clinic_id = NEW.clinic_id ORDER BY ends_at DESC LIMIT 1;

        IF _sub_active IS DISTINCT FROM TRUE OR _sms_enabled IS DISTINCT FROM TRUE THEN RETURN NEW; END IF;

        SELECT balance INTO _wallet_balance FROM clinic_wallets WHERE clinic_id = NEW.clinic_id;
        IF _wallet_balance IS NULL OR _wallet_balance < 0.50 THEN RETURN NEW; END IF;

        -- B. Rate Limit (10 mins)
        IF EXISTS (SELECT 1 FROM communication_logs WHERE appointment_id = NEW.id AND created_at > NOW() - INTERVAL '10 minutes') THEN 
            RETURN NEW; 
        END IF;

        -- C. Fetch Dynamic Configuration
        -- 1. WhatsApp Integration (Sender ID)
        SELECT jsonb_build_object('phone_number_id', phone_number_id, 'sender_number', sender_phone_number) 
        INTO _integration 
        FROM clinic_integrations
        WHERE clinic_id = NEW.clinic_id AND provider_id = 'fast2sms_whatsapp' AND is_enabled = true;

        -- 2. Template ID
        SELECT message_id INTO _template_id
        FROM clinic_message_templates
        WHERE clinic_id = NEW.clinic_id AND event_trigger = 'prescription_ready' AND is_active = true;

        IF _integration IS NULL OR _template_id IS NULL THEN RETURN NEW; END IF;

        -- D. Build Payload
        _meta := public._internal_get_notification_meta(NEW.id);
        
        -- Logic for Var5 (History Button): Suffix is "UHID+PHONE"
        _history_suffix := TRIM(_meta->>'patient_uhid') || '+' || TRIM(_meta->>'patient_phone');
        
        -- ✅ NEW: Logic for Var4 (PDF Download): Full R2 path "{clinic_name}/{appt_id}.pdf"
        _pdf_path := REPLACE(TRIM(_meta->>'clinic_name'), ' ', '_') || '/' || NEW.id || '.pdf';

        _payload := jsonb_build_object(
            'event', 'prescription_ready',
            'clinic_id', NEW.clinic_id,
            'appointment_id', NEW.id,
            'target_phone', _meta->>'patient_phone',
            'cost', 0.33,
            'provider_config', _integration,
            'template_id', _template_id, 
            'variables', jsonb_build_object(
                'Var1', _meta->>'patient_name',
                'Var2', _meta->>'doctor_name',
                'Var3', _meta->>'clinic_name',
                'Var4', _pdf_path,       -- ✅ CHANGED: Full path for PDF Download button
                'Var5', _history_suffix  -- History Link Suffix
            )
        );
...
DECLARE
    ...
    _payload JSONB;
BEGIN
    -- Trigger on Time Change (Future only)
    IF NEW.start_time IS DISTINCT FROM OLD.start_time AND NEW.start_time > NOW() THEN
        ...
        -- C. Dynamic Config
        SELECT jsonb_build_object('phone_number_id', phone_number_id, 'sender_number', sender_phone_number) 
        INTO _integration FROM clinic_integrations
        WHERE clinic_id = NEW.clinic_id AND provider_id = 'fast2sms_whatsapp' AND is_enabled = true;
        
        SELECT message_id INTO _template_id
        FROM clinic_message_templates
        WHERE clinic_id = NEW.clinic_id AND event_trigger = 'reschedule_alert' AND is_active = true;
        ...
        _payload := jsonb_build_object(
            ...
            'variables', jsonb_build_object(
                'Var1', _meta->>'patient_name',
                'Var2', _meta->>'doctor_name',
                'Var3', _meta->>'appt_date_formatted',
                'Var4', _meta->>'appt_time_formatted',
                'Var5', _meta->>'clinic_name',
                'Var6', _meta->>'clinic_phone'
            )
        );
...
--
-- Name: process_daily_reminders(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.process_daily_reminders() RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
    -- Optimized JOIN query to fetch everything in one go
    FOR _appt IN 
        SELECT 
            a.id AS appt_id,
            a.clinic_id,
            ci.phone_number_id,
            ci.sender_phone_number,
            cmt.message_id AS template_id
        FROM appointments a
        JOIN clinics c ON a.clinic_id = c.id
        -- Active Subscription
        JOIN clinic_subscriptions sub ON sub.clinic_id = c.id 
            AND sub.status = 'active' 
            AND sub.ends_at > NOW() 
            AND sub.limit_sms_enabled = true
        -- Sufficient Wallet
        JOIN clinic_wallets w ON w.clinic_id = c.id 
            AND w.balance >= 0.50
        -- Enabled Integration
        JOIN clinic_integrations ci ON ci.clinic_id = c.id 
            AND ci.provider_id = 'fast2sms_whatsapp' 
            AND ci.is_enabled = true
        -- Configured Template
        JOIN clinic_message_templates cmt ON cmt.clinic_id = c.id
            AND cmt.event_trigger = 'appointment_reminder'
            AND cmt.is_active = true
        WHERE 
            a.start_time BETWEEN NOW() AND (NOW() + INTERVAL '24 hours')
            AND a.status = 'scheduled'
            AND a.is_reminder_sent = false
        LIMIT 200
    LOOP
        _meta := public._internal_get_notification_meta(_appt.appt_id);

        _payload := jsonb_build_object(
            'event', 'appointment_reminder',
            'clinic_id', _appt.clinic_id,
            'appointment_id', _appt.appt_id,
            'target_phone', _meta->>'patient_phone',
            'cost', 0.33,
            'provider_config', jsonb_build_object(
                'phone_number_id', _appt.phone_number_id,
                'sender_number', _appt.sender_phone_number
            ),
            'template_id', _appt.template_id, 
            'variables', jsonb_build_object(
                'Var1', _meta->>'patient_name',
                'Var2', _meta->>'doctor_name',
                'Var3', _meta->>'clinic_name',
                'Var4', _meta->>'appt_date_formatted',
                'Var5', _meta->>'appt_time_formatted'
            )
        );
        ...
        WHERE id = _appt.appt_id;
        
        _processed_count := _processed_count + 1;
    END LOOP;
    
    RAISE NOTICE 'Processed % reminders.', _processed_count;
END;
$$;
...
--
-- Name: process_wallet_topup(uuid, numeric, text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.process_wallet_topup(_clinic_id uuid, _amount numeric, _ref_id text, _desc text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
    ...
    _new_balance numeric;
BEGIN
    -- Strict Gate: Only Super Admin
    IF NOT public.is_super_admin() THEN 
        RAISE EXCEPTION 'Access Denied: Only Super Admin can process manual top-ups.'; 
    END IF;

    SELECT id INTO _wallet_id FROM public.clinic_wallets WHERE clinic_id = _clinic_id;
    ...
    INSERT INTO public.clinic_wallet_transactions (
        wallet_id, amount, type, reference_id, description, balance_after, created_by
    ) VALUES (
        _wallet_id, _amount, 'topup', _ref_id, _desc, _new_balance, auth.uid()
    );
...
CREATE FUNCTION public.quick_register_patient(_full_name text, _phone text, _age integer, _gender public.gender_type, _clinic_id uuid DEFAULT NULL::uuid, _address_street text DEFAULT NULL::text, _district text DEFAULT NULL::text, _state text DEFAULT NULL::text, _pincode text DEFAULT NULL::text, _religion text DEFAULT NULL::text, _dob date DEFAULT NULL::date, _access_flags jsonb DEFAULT '{"critical": false, "wheelchair": false}'::jsonb) RETURNS uuid
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
    ...
    IF public.auth_user_role() NOT IN ('staff', 'admin', 'doctor') AND NOT public.is_super_admin() THEN 
        RAISE EXCEPTION 'Unauthorized: Only staff/doctors can register patients.';
    END IF;

    IF _resolved_id IS NULL THEN RAISE EXCEPTION 'Context missing'; END IF;

    IF _phone IS NOT NULL AND _phone != '' THEN
        SELECT id INTO _new_id FROM patients WHERE phone = _phone AND clinic_id = _resolved_id LIMIT 1;
    ...
    INSERT INTO patients (
        clinic_id, full_name, phone, age, gender, uhid, 
        address_street, district, state, pincode, religion, dob, access_flags
    )
    VALUES (
        _resolved_id, _full_name, _phone, _age, _gender, upper(_uhid), 
        _address_street, _district, _state, _pincode, _religion, _dob, _access_flags
    )
    RETURNING id INTO _new_id;
...
BEGIN
    ...
    -- 2. Provision FREE TRIAL Subscription
    ...
    -- 5. [NEW] Configure Default Message Templates
    INSERT INTO clinic_message_templates (clinic_id, event_trigger, message_id)
    VALUES 
        (_new_clinic_id, 'prescription_ready', '10239'),
        (_new_clinic_id, 'reschedule_alert', '10143'),
        (_new_clinic_id, 'appointment_reminder', '10141');
...
--
-- Name: save_document_intelligence(uuid, text, text, jsonb, jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.save_document_intelligence(_document_id uuid, _full_text text, _ai_summary text, _clinical_data jsonb, _tags jsonb) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
    _clinic_id uuid;
BEGIN
    SELECT p.clinic_id INTO _clinic_id 
    FROM patient_documents d
    JOIN patients p ON d.patient_id = p.id
    WHERE d.id = _document_id;

    INSERT INTO patient_document_content (
        document_id, clinic_id, full_text_content, ai_summary, clinical_data, semantic_tags
    ) VALUES (
        _document_id, _clinic_id, _full_text, _ai_summary, _clinical_data, _tags
    )
...
BEGIN
    ...
    _cleaned_query := TRIM(_query);

    -- B. Early Exit (Don't query DB for empty strings)
    IF _cleaned_query = '' OR _cleaned_query IS NULL THEN 
        RETURN; 
    END IF;

    -- C. Construct "Prefix-Friendly" Search Query
    -- Converts "para 500" -> "para & 500:*" 
    -- This allows finding "Paracetamol" when typing just "Para"
    BEGIN
        _ts_query := plainto_tsquery('english', _cleaned_query) || to_tsquery('english', ':*');
    EXCEPTION WHEN OTHERS THEN
        -- Fallback for special chars that might break tsquery syntax
        _ts_query := plainto_tsquery('english', _cleaned_query);
    END;

    -- D. Perform Hybrid Search
    RETURN QUERY
    SELECT 
        m.id, 
        m.name AS brand_name,
        m.salt_composition AS generic_name,
        -- Optimize String Concat: Pre-trim to save CPU cycles
    ...
    WHERE 
        ...
        AND (
            -- 1. FASTEST: Exact Brand Prefix (Uses B-Tree Index)
            -- Catches "Dolo" -> "Dolo 650" instantly
            m.name ILIKE _cleaned_query || '%'
            
            OR
            
            -- 2. FAST: Full Text Search (Uses GIN Vector Index)
            -- Catches "Para 500" -> "Paracetamol 500mg"
            m.search_vector @@ _ts_query
            
            OR
            
            -- 3. FUZZY: Generic Name Infix (Uses Trigram Index)
            -- Only run if query is substantial (> 2 chars) to avoid slow scans
            (LENGTH(_cleaned_query) > 2 AND m.salt_composition ILIKE '%' || _cleaned_query || '%')
        )
    ORDER BY 
        -- Rank 1: Brand Prefix Match (User likely wants the brand they typed)
        CASE WHEN m.name ILIKE _cleaned_query || '%' THEN 0 ELSE 1 END ASC,
        
        -- Rank 2: Relevance Score (Full Text Rank)
        ts_rank(m.search_vector, _ts_query) DESC,
...
CREATE FUNCTION public.search_patients_fuzzy(_query text, _clinic_id uuid DEFAULT NULL::uuid) RETURNS TABLE(id uuid, full_name text, phone text, uhid text, age integer, gender text, last_visit date)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
    -- 1. Context & Security
    _resolved_clinic_id := COALESCE(_clinic_id, public.auth_my_clinic_id());
    ...
    FROM patients p
    WHERE p.clinic_id = _resolved_clinic_id
      AND (
        p.phone ILIKE _query || '%' OR            -- Prefix match for Phone (Fast)
        p.uhid ILIKE '%' || _query OR             -- Suffix/Contains match for UHID (Matches original logic)
        p.full_name ILIKE '%' || _query || '%'    -- Fuzzy match for Name
      )
    LIMIT 15;
END;
$$;
...
BEGIN
    ...
    SELECT appointment_id, doctor_id INTO _appt_id, _doctor_id
    FROM consultations
    ...
    -- 4. Overwrite Medicines (Clean State)
    ...
    -- 6. Close Appointment
    UPDATE appointments 
    SET status = 'completed', consult_ended_at = NOW() 
    WHERE id = _appt_id;
...
--
-- Name: trg_auto_create_medical_profile(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.trg_auto_create_medical_profile() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    INSERT INTO public.patient_medical_profiles (patient_id)
    VALUES (NEW.id)
...
--
-- Name: trg_auto_generate_patient_details(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.trg_auto_generate_patient_details() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    -- Check if UHID is blank
    IF NEW.uhid IS NULL OR NEW.uhid = '' THEN
        -- Format: UHID + YYMMDD + 6-Digit-Counter
        -- Example: UHID260109000001, UHID260109000002...
        -- This is mathematically guaranteed to be unique.
        NEW.uhid := 'UHID' || to_char(now(), 'YYMMDD') || lpad(nextval('uhid_seq')::text, 6, '0');
    END IF;

    -- Defaults
    IF NEW.gender IS NULL THEN NEW.gender := 'Male'; END IF; 
    IF NEW.access_flags IS NULL THEN NEW.access_flags := '{"critical": false, "wheelchair": false}'::jsonb; END IF;
...
--
-- Name: trg_create_clinic_wallet(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.trg_create_clinic_wallet() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
-- Name: trigger_auto_analyze_patterns(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.trigger_auto_analyze_patterns() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
    IF NEW.doctor_id IS NOT NULL THEN
        BEGIN
            PERFORM public.analyze_doctor_patterns(NEW.doctor_id);
...
BEGIN
    ...
    INSERT INTO patient_medical_profiles (
        patient_id, allergies, chronic_conditions, lifestyle_tags, family_history, updated_at
    ) VALUES (
        _patient_id,
    ...
    RETURN jsonb_build_object('success', true, 'updated_at', result_record.updated_at);
END;
$$;
...
CREATE TABLE public.clinic_integrations (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    clinic_id uuid NOT NULL,
    provider_id text DEFAULT 'fast2sms_whatsapp'::text,
...
CREATE TABLE public.clinic_invitations (
    ...
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT clinic_invitations_status_check CHECK ((status = ANY (ARRAY['pending'::text, 'accepted'::text, 'expired'::text])))
);


--
-- Name: clinic_message_templates; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.clinic_message_templates (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    clinic_id uuid,
...
--
-- Name: clinic_wallet_transactions; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.clinic_wallet_transactions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
--
-- Name: clinic_wallets; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.clinic_wallets (
    ...
    updated_at timestamp with time zone DEFAULT now(),
    CONSTRAINT positive_balance_check CHECK ((balance >= (0)::numeric))
);
...
--
-- Name: doctor_clinical_patterns; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.doctor_clinical_patterns (
    doctor_id uuid NOT NULL,
    clinic_id uuid,
    top_diagnoses jsonb DEFAULT '[]'::jsonb,
    prescribing_habits jsonb DEFAULT '{}'::jsonb,
...
--
-- Name: patient_document_content; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.patient_document_content (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    document_id uuid,
    clinic_id uuid,
...
--
-- Name: uhid_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: view_audit_logs_readable; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.view_audit_logs_readable AS
 SELECT al.id,
    al.created_at,
    al.action,
    al.category,
...
--
-- Name: clinic_integrations clinic_integrations_clinic_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_integrations
    ADD CONSTRAINT clinic_integrations_clinic_unique UNIQUE (clinic_id, provider_id);
...
ALTER TABLE ONLY public.clinic_integrations
    ADD CONSTRAINT clinic_integrations_pkey PRIMARY KEY (id);


--
-- Name: clinic_invitations clinic_invitations_clinic_id_email_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_invitations
    ADD CONSTRAINT clinic_invitations_clinic_id_email_key UNIQUE (clinic_id, email);
...
--
-- Name: clinic_message_templates clinic_message_templates_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_message_templates
    ADD CONSTRAINT clinic_message_templates_pkey PRIMARY KEY (id);
...
--
-- Name: clinic_wallet_transactions clinic_wallet_transactions_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_wallet_transactions
    ADD CONSTRAINT clinic_wallet_transactions_pkey PRIMARY KEY (id);


--
-- Name: clinic_wallets clinic_wallets_clinic_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_wallets
    ADD CONSTRAINT clinic_wallets_clinic_id_key UNIQUE (clinic_id);


--
-- Name: clinic_wallets clinic_wallets_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_wallets
    ADD CONSTRAINT clinic_wallets_pkey PRIMARY KEY (id);
...
--
-- Name: doctor_clinical_patterns doctor_clinical_patterns_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.doctor_clinical_patterns
    ADD CONSTRAINT doctor_clinical_patterns_pkey PRIMARY KEY (doctor_id);
...
--
-- Name: patient_document_content patient_document_content_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.patient_document_content
    ADD CONSTRAINT patient_document_content_pkey PRIMARY KEY (id);
...
--
-- Name: clinic_message_templates unique_trigger_per_clinic; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_message_templates
    ADD CONSTRAINT unique_trigger_per_clinic UNIQUE (clinic_id, event_trigger);
...
CREATE INDEX idx_audit_category ON public.audit_logs USING btree (category);


--
-- Name: idx_audit_clinic_created; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_audit_clinic_created ON public.audit_logs USING btree (clinic_id, created_at DESC);
...
CREATE INDEX idx_audit_resource ON public.audit_logs USING btree (resource_id);
...
--
-- Name: idx_doc_content_search; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_doc_content_search ON public.patient_document_content USING gin (to_tsvector('english'::regconfig, full_text_content));


--
-- Name: idx_doc_content_unique; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX idx_doc_content_unique ON public.patient_document_content USING btree (document_id);
...
--
-- Name: idx_wallet_txns_created_at; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_wallet_txns_created_at ON public.clinic_wallet_transactions USING btree (created_at DESC);
...
CREATE INDEX idx_wallet_txns_wallet_id ON public.clinic_wallet_transactions USING btree (wallet_id);
...
--
-- Name: clinics on_clinic_created_create_wallet; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER on_clinic_created_create_wallet AFTER INSERT ON public.clinics FOR EACH ROW EXECUTE FUNCTION public.trg_create_clinic_wallet();


--
-- Name: patients on_patient_created_profile; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER on_patient_created_profile AFTER INSERT ON public.patients FOR EACH ROW EXECUTE FUNCTION public.trg_auto_create_medical_profile();


--
-- Name: patients on_patient_import_defaults; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER on_patient_import_defaults BEFORE INSERT ON public.patients FOR EACH ROW EXECUTE FUNCTION public.trg_auto_generate_patient_details();
...
CREATE TRIGGER trg_audit_appointments AFTER INSERT OR DELETE OR UPDATE ON public.appointments FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();


--
-- Name: clinic_api_keys trg_audit_clinic_api_keys; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_clinic_api_keys AFTER INSERT OR DELETE OR UPDATE ON public.clinic_api_keys FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();


--
-- Name: clinic_subscriptions trg_audit_clinic_subscriptions; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_clinic_subscriptions AFTER INSERT OR DELETE OR UPDATE ON public.clinic_subscriptions FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();


--
-- Name: clinic_wallets trg_audit_clinic_wallets; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_clinic_wallets AFTER INSERT OR DELETE OR UPDATE ON public.clinic_wallets FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();


--
-- Name: clinics trg_audit_clinics; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_clinics AFTER INSERT OR DELETE OR UPDATE ON public.clinics FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();


--
-- Name: consultations trg_audit_consultations; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_consultations AFTER INSERT OR DELETE OR UPDATE ON public.consultations FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();


--
-- Name: doctors trg_audit_doctors; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_doctors AFTER INSERT OR DELETE OR UPDATE ON public.doctors FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();


--
-- Name: invoice_payments trg_audit_invoice_payments; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_audit_invoice_payments AFTER INSERT OR DELETE OR UPDATE ON public.invoice_payments FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();
...
CREATE TRIGGER trg_audit_invoices AFTER INSERT OR DELETE OR UPDATE ON public.invoices FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();
...
CREATE TRIGGER trg_audit_patients AFTER INSERT OR DELETE OR UPDATE ON public.patients FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();
...
CREATE TRIGGER trg_audit_profiles AFTER INSERT OR DELETE OR UPDATE ON public.profiles FOR EACH ROW EXECUTE FUNCTION public.enhanced_audit_log();
...
--
-- Name: communication_logs trg_deduct_wallet; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_deduct_wallet AFTER INSERT ON public.communication_logs FOR EACH ROW EXECUTE FUNCTION public.handle_wallet_deduction_on_log();
...
CREATE TRIGGER trg_notify_prescription AFTER UPDATE OF status ON public.appointments FOR EACH ROW EXECUTE FUNCTION public.notify_prescription_ready();
...
--
-- Name: appointments trg_update_doctor_patterns; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_update_doctor_patterns AFTER UPDATE OF status ON public.appointments FOR EACH ROW WHEN (((new.status = 'completed'::public.appointment_status) AND (old.status IS DISTINCT FROM 'completed'::public.appointment_status))) EXECUTE FUNCTION public.trigger_auto_analyze_patterns();
...
--
-- Name: clinic_message_templates clinic_message_templates_clinic_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_message_templates
    ADD CONSTRAINT clinic_message_templates_clinic_id_fkey FOREIGN KEY (clinic_id) REFERENCES public.clinics(id) ON DELETE CASCADE;
...
--
-- Name: clinic_wallet_transactions clinic_wallet_transactions_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_wallet_transactions
    ADD CONSTRAINT clinic_wallet_transactions_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.profiles(id);


--
-- Name: clinic_wallet_transactions clinic_wallet_transactions_wallet_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_wallet_transactions
    ADD CONSTRAINT clinic_wallet_transactions_wallet_id_fkey FOREIGN KEY (wallet_id) REFERENCES public.clinic_wallets(id);


--
-- Name: clinic_wallets clinic_wallets_clinic_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinic_wallets
    ADD CONSTRAINT clinic_wallets_clinic_id_fkey FOREIGN KEY (clinic_id) REFERENCES public.clinics(id) ON DELETE CASCADE;
...
--
-- Name: doctor_clinical_patterns doctor_clinical_patterns_clinic_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.doctor_clinical_patterns
    ADD CONSTRAINT doctor_clinical_patterns_clinic_id_fkey FOREIGN KEY (clinic_id) REFERENCES public.clinics(id);


--
-- Name: doctor_clinical_patterns doctor_clinical_patterns_doctor_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.doctor_clinical_patterns
    ADD CONSTRAINT doctor_clinical_patterns_doctor_id_fkey FOREIGN KEY (doctor_id) REFERENCES public.profiles(id);
...
--
-- Name: patient_document_content patient_document_content_clinic_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.patient_document_content
    ADD CONSTRAINT patient_document_content_clinic_id_fkey FOREIGN KEY (clinic_id) REFERENCES public.clinics(id);


--
-- Name: patient_document_content patient_document_content_document_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.patient_document_content
    ADD CONSTRAINT patient_document_content_document_id_fkey FOREIGN KEY (document_id) REFERENCES public.patient_documents(id) ON DELETE CASCADE;
...
--
-- Name: clinic_invitations Admins manage invitations; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins manage invitations" ON public.clinic_invitations USING (((clinic_id = public.auth_my_clinic_id()) AND (public.auth_user_role() = 'admin'::public.user_role))) WITH CHECK (((clinic_id = public.auth_my_clinic_id()) AND (public.auth_user_role() = 'admin'::public.user_role)));
...
--
-- Name: audit_logs Clinic Admins View Own Logs; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Clinic Admins View Own Logs" ON public.audit_logs FOR SELECT USING (((public.auth_user_role() = 'admin'::public.user_role) AND (clinic_id = public.auth_my_clinic_id())));


--
-- Name: clinic_integrations Clinic View Own Integrations; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Clinic View Own Integrations" ON public.clinic_integrations FOR SELECT USING ((clinic_id = public.auth_my_clinic_id()));


--
-- Name: clinic_wallet_transactions Clinic View Own Transactions; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Clinic View Own Transactions" ON public.clinic_wallet_transactions FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.clinic_wallets w
  WHERE ((w.id = clinic_wallet_transactions.wallet_id) AND (w.clinic_id = public.auth_my_clinic_id())))));


--
-- Name: clinic_wallets Clinic View Own Wallet; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Clinic View Own Wallet" ON public.clinic_wallets FOR SELECT USING ((clinic_id = public.auth_my_clinic_id()));
...
--
-- Name: patient_document_content Content Isolation; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Content Isolation" ON public.patient_document_content USING ((public.is_super_admin() OR (clinic_id = public.auth_my_clinic_id())));
...
--
-- Name: doctor_clinical_patterns Pattern Isolation; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Pattern Isolation" ON public.doctor_clinical_patterns USING ((public.is_super_admin() OR (clinic_id = public.auth_my_clinic_id())));
...
CREATE POLICY "Public read plans" ON public.plan_master FOR SELECT USING (true);
...
CREATE POLICY "SA Manage Integrations" ON public.clinic_integrations USING (public.is_super_admin()) WITH CHECK (public.is_super_admin());


--
-- Name: clinic_wallet_transactions SA Manage Transactions; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "SA Manage Transactions" ON public.clinic_wallet_transactions USING (public.is_super_admin()) WITH CHECK (public.is_super_admin());


--
-- Name: clinic_wallets SA Manage Wallets; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "SA Manage Wallets" ON public.clinic_wallets USING (public.is_super_admin()) WITH CHECK (public.is_super_admin());
...
--
-- Name: audit_logs Super Admin View All Logs; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Super Admin View All Logs" ON public.audit_logs FOR SELECT USING (public.is_super_admin());
...
--
-- Name: clinic_wallet_transactions; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.clinic_wallet_transactions ENABLE ROW LEVEL SECURITY;

--
-- Name: clinic_wallets; Type: ROW SECURITY; Schema: public; Owner: -
--
...
--
-- Name: doctor_clinical_patterns; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.doctor_clinical_patterns ENABLE ROW LEVEL SECURITY;
...
--
-- Name: patient_document_content; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.patient_document_content ENABLE ROW LEVEL SECURITY;
...
\unrestrict CZh2b5R9RD6um9kIo2ijtm3h41IcgSiwkzbzN0xrx92SXVfmIyHKRb96yAtpcfi
